...
const Partners = (props) => {
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/partners/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultPartner(selectedRowKeys);
  };
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх хамтрагч</h3>
              </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    pagination: state.partnerReducer.paginationLast,
    excelData: state.partnerReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadPartner: (query) => dispatch(actions.loadPartner(query)),
    deleteMultPartner: (ids) => dispatch(actions.deleteMultPartner(ids)),
